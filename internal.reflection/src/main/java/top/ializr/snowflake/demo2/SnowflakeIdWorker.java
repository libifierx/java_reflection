...
public class SnowflakeIdWorker {
    ...
    public synchronized long nextId() {
        ...
        if (lastTimestamp == timestamp) {
        	/**
        	 * comment by zhenghao:   sequenceMask = 4095 = 1111 1111 1111
        	 * 数值 & sequenceMask 结果总是在[0,4095]，即使数值为负数也一样。
        	 * 
        	 * 这里的意思是：在X毫秒时，计算序列结果是1,2,3,...,4095
        	 * 				如果计算序列结果0时，则时间等到X+1毫秒
        	 * 				
        	 *              在X+1毫秒时，计算序列结果是1,2,3,...,4095
        	 *              如果计算序列结果0时，则时间等到X+2毫秒
        	 * 计算序列的结果范围[0,4095] 共计4096个，即2的12次幂。
        	 */
